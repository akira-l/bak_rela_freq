...
class cls_model(nn.Module): 
    def __init__(self):
        ...
        self.vec_size = 300
        self.obj_linear1 = Linear(self.vec_size, 200)
        self.obj_linear2 = Linear(200, 100)
        self.obj_linear3 = Linear(110, 240)

        self.sub_linear1 = Linear(self.vec_size, 200)
        self.sub_linear2 = Linear(200, 100)
        self.sub_linear3 = Linear(110, 240)

        self.rela_linear1 = Linear(480+10, 600)
        self.rela_linear2 = Linear(600, 300)
        self.rela_linear3 = Linear(300, rela_set_num, bias=False)
        self.drop = 0.3
    ...
    def forward(self, x, training=True):
        ...
        pos = x[:, 2, self.vec_size:]
        obj_l1 = F.dropout(self.obj_linear1(obj), p=self.drop, training=training)
        obj_l2 =self.obj_linear2(obj_l1)
        obj_l3 = self.obj_linear3(torch.cat([obj_l2, obj_pos], 1))

        sub_l1 = F.dropout(self.sub_linear1(sub), p=self.drop, training=training)
        sub_l2 = self.sub_linear2(sub_l1)
        sub_l3 = self.sub_linear3(torch.cat([sub_l2, sub_pos], 1))

        rela_in = torch.cat([obj_l3, sub_l3, pos], 1)
...
class cls_model(nn.Module): 
    def __init__(self):
        ...
        self.obj_pos_l = Linear(10, 60)
        self.obj_linear2 = Linear(260, 170)

        self.sub_linear1 = Linear(300, 200)
        self.sub_pos_l = Linear(10, 60)
        self.sub_linear2 = Linear(260, 170)

        self.rela_linear1 = Linear(340+20, 240)
...
def train(args, train_data):
    ...
    obj_loca = torch.zeros(len(src_data), 10)
    obj_loca[:, :4] = src_data[:, 2:6]/1024
    ...
    sub_loca = torch.zeros(len(src_data), 10)
    sub_loca[:, :4] = src_data[:, 6:10]/1024
    ...
    rela_tensor = torch.cat([torch.zeros(src_data.size(0), 300), src_data[:, 10:-1]], 1)
    ...
    for epoch_counter in range(epoch):
        ...
        for iter_counter in range(iteration):
            ...
            #in_tmp = inputs[given_idx[iter_counter*bs:(iter_counter+1)*bs]]
...
def get_rela_name():
    tmp_dict = torch.load('save_dict150-20.pt')
    return tmp_dict['ind_to_predicates']
...
if __name__ == '__main__':
    ...
    vec_dict = torch.load('150-50vec.pt')
    obj_vec = torch.cat(vec_dict['obj_vec'], 0)
    rela_vec = torch.cat(vec_dict['rela_vec'], 0)

    #ylb_tmp = torch.load('150-50vec-ylb.pt')
    #obj_vec = torch.stack(ylb_tmp['obj_vec'])
    ...
    pass_counter = 0
    for rela_num in range(51):
        if rela_prec_dict[rela_num] > 0:
            pass_counter += 1
    rela_name = get_rela_name()
    for print_ind in range(len(rela_name)):
        print('acc: %.4f   relation: %s  ' % (rela_prec_dict[print_ind], rela_name[print_ind]))
    print('kill %d / 50' % pass_counter)
